...
8 agents: JD Parser, Retrieval, Match, Engagement, Interest,
          Process (orchestrator), Ranking, Explainability
"""
import logging
...
Return JSON with exactly these fields:
{
  ...
  "keywords": ["keyword1", "keyword2", ...]
    ...
    logger.info(f"JD Parser: role={parsed['role']}  skills={len(parsed['skills'])}")
    return {**state, "parsed_jd": parsed, "pipeline_status": "jd_parsed"}


# ──────────────────────────────────────────────────────────
# NODE 2: Retrieval — always fetches ALL candidates
# ──────────────────────────────────────────────────────────
async def retrieval_node(state: AgentState) -> AgentState:
    logger.info("Retrieval: fetching ALL candidates from Qdrant")

    jd    = state["parsed_jd"]
    query = f"{jd['role']} {' '.join(jd['skills'])} {' '.join(jd['keywords'])}"

    # No top_k — search_candidates always returns everything
    results = vector_store.search_candidates(query)
    logger.info(f"Retrieval: fetched {len(results)} candidates")

    return {**state, "retrieved_candidates": results, "pipeline_status": "retrieved"}


# ──────────────────────────────────────────────────────────
# NODE 3: Match Agent  (called per-candidate inside NODE 6)
# ──────────────────────────────────────────────────────────
async def match_agent(jd: dict, candidate: dict, cosine_score: float) -> dict:
    ...
    # cosine is low-signal (1-dim hash) so skill overlap carries more weight
    match_score = round(
        min(max(cosine_score * 0.2 + skill_overlap * 0.5 + exp_score * 0.3, 0.0), 1.0),
        3
    )

    system = """You are a technical recruiter. Write a concise 2-sentence match explanation.
...
# ──────────────────────────────────────────────────────────
# NODE 4: Engagement Agent  (called per-candidate inside NODE 6)
# ──────────────────────────────────────────────────────────
...
Respond naturally and authentically. Show genuine personality.
Background: {candidate.get('summary', 'Experienced professional')}"""
    ...
    for q in questions:
        ...
        conversation.append({"role": "recruiter", "message": q})
        conversation.append({"role": "candidate", "message": answer.strip()})

    return conversation


# ──────────────────────────────────────────────────────────
# NODE 5: Interest Agent  (called per-candidate inside NODE 6)
# ──────────────────────────────────────────────────────────
async def interest_agent(conversation: List[dict], candidate: dict, jd: dict) -> dict:
    ...
    system = """You are an expert talent psychologist analyzing candidate interest.
Score on:
...
# ──────────────────────────────────────────────────────────
# NODE 6: Process ALL Candidates
# ──────────────────────────────────────────────────────────
async def process_candidates_node(state: AgentState, db: AsyncSession) -> AgentState:
    from db.models import Candidate

    total = len(state["retrieved_candidates"])
    logger.info(f"Processing ALL {total} candidates")
    results: List[CandidateResult] = []

    for i, retrieved in enumerate(state["retrieved_candidates"]):
        cid          = retrieved.get("candidate_id")
        cosine_score = retrieved.get("score", 0.5)

        logger.info(f"  [{i+1}/{total}] evaluating {cid}")

        db_result    = await db.execute(select(Candidate).where(Candidate.id == cid))
        db_candidate = db_result.scalar_one_or_none()

        if not db_candidate:
            logger.warning(f"  Candidate {cid} not in DB — skipping")
            continue
        ...
        try:
            ...
            logger.error(f"  Error on {cid}: {e}")
            state["errors"].append(f"Candidate {cid}: {str(e)}")

    logger.info(f"Processed {len(results)}/{total} candidates successfully")
    return {**state, "candidate_results": results, "pipeline_status": "candidates_processed"}


# ──────────────────────────────────────────────────────────
# NODE 7: Ranking
# Sorts all candidates, then applies shortlist_threshold filter.
# shortlist_threshold = 0.0  → return everyone
# shortlist_threshold = 0.6  → only keep final_score >= 60%
# ──────────────────────────────────────────────────────────
async def ranking_node(state: AgentState) -> AgentState:
    threshold = state.get("shortlist_threshold", 0.0)
    ...
    shortlisted = (
        [c for c in all_ranked if c["final_score"] >= threshold]
        if threshold > 0 else all_ranked
    )

    logger.info(f"Total: {len(all_ranked)}, Shortlisted: {len(shortlisted)}")

    return {
        **state,
        "ranked_results": all_ranked,       # ✅ ALL candidates
        "shortlisted": shortlisted,         # ✅ ONLY shortlisted
        "pipeline_status": "ranked"
    }


# ──────────────────────────────────────────────────────────
# NODE 8: Explainability
# ──────────────────────────────────────────────────────────
async def explainability_node(state: AgentState) -> AgentState:
    enriched = []

    for candidate in state["ranked_results"]:
        system = """You are a senior talent advisor. Write a recruiter-friendly summary (3-4 sentences)
...
Interest Notes: {candidate['interest_reasoning']}"""
    ...
    logger.info(f"Explainability: done for {len(enriched)} candidates")
